...
async def cleanup_expired_unverified_users_test(db):
    stmt = delete(User).where(User.status == UserStatus.EXPIRED)
    result = cast(
...
def cleanup_expired_unverified_users(db):
    stmt = delete(User).where(User.status == UserStatus.EXPIRED)
    result = cast(
...
def cleanup_expired_unverified_users_task(self):
    logger.info(
        "cleanup_expired_unverified_users_task_start",
    ...
    logger.info(
        "cleanup_expired_unverified_users_end", deleted_count=deleted_count
    )
    return deleted_count
